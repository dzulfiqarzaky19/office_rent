    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-24">
      {TESTIMONIALS.map((t, i) => (
        <FadeIn key={t.name} delay={i * 0.12} y={20}>
          <p className="font-prada text-[11px] text-black mb-12 leading-loose tracking-widest uppercase">
            &quot;{t.text}&quot;
          </p>
          <div className="flex flex-col gap-2 pt-8 border-t border-border-light">
            <p className="font-prada text-[10px] font-bold tracking-[0.3em] text-black">
              {t.name}
            </p>
            <p className="font-prada text-[9px] text-text-muted tracking-[0.2em]">
              {t.role}
            </p>
          </div>
        </FadeIn>
