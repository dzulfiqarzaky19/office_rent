...
export default function SummaryPanel() {
  ...
  const monthlySavings = (() => {
    if (rentalPeriod !== "monthly") return 0;
    const items = [selectedDesk, selectedChair, ...accessories].filter(Boolean);
    const weeklyTotal = items.reduce((sum, item) => sum + (item?.weeklyPrice || 0) * 4, 0);
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white">
      ...
      {/* Pricing toggle — minimalist Prada style */}
        ...
        <AnimatePresence mode="popLayout">
          ...
          {selectedDesk && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="flex items-center justify-between py-3 border-b border-border-light min-w-0 gap-4"
            >
              ...
              <span className="font-prada text-[10px] text-black tracking-tighter shrink-0">
                ${rentalPeriod === "weekly" ? selectedDesk.weeklyPrice : selectedDesk.monthlyPrice}
              </span>
            </motion.div>
          )}
          ...
          {selectedChair && (
            ...
            >
              ...
              <span className="font-prada text-[10px] text-black tracking-tighter shrink-0">
                ${rentalPeriod === "weekly" ? selectedChair.weeklyPrice : selectedChair.monthlyPrice}
              </span>
            </motion.div>
          )}
          ...
          {accessories.map((acc) => (
            ...
            >
              ...
              <span className="font-prada text-[10px] text-black tracking-tighter shrink-0">
                ${rentalPeriod === "weekly" ? acc.weeklyPrice : acc.monthlyPrice}
              </span>
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      <div className="mt-8 pt-6 border-t border-black shrink-0">
        {rentalPeriod === "monthly" && monthlySavings > 0 && (
          <motion.div
            ...
            className="flex items-center justify-between mb-4 border border-border-light px-2 py-1"
          >
